...
使用方式：
    ...
    python ablation_study.py --tickers 0700 9988 0005 --days 800
"""
...
from strategy.market import CURRENCY, DEFAULT_BENCHMARK, DEFAULT_BUY_COST, DEFAULT_SELL_COST
from strategy.universe import DEFAULT_TICKERS
...
def main():
    ...
    # 下載 benchmark 用於 regime filter（對齊 README）
    from strategy.benchmark import fetch_benchmark
    market_close = fetch_benchmark(DEFAULT_BENCHMARK, days=args.days)
    if len(market_close) == 0:
        market_close = None
        print('⚠️ 無法下載 benchmark，regime filter 停用')

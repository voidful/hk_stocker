...
使用方式:
  ...
  python3 sector_rotation_report.py --compare            # 與 2800.HK 比較
"""
...
from strategy.us_market import fetch_us_signals, align_us_to_hk
...
from strategy.market import DEFAULT_BENCHMARK, DEFAULT_BENCHMARK_LABEL
from strategy.universe import EXTENDED_TICKERS

def get_tickers():
    """取得擴展股池。"""
    try:
        from ai_report import EXTENDED_TICKERS as AI_REPORT_TICKERS
        return AI_REPORT_TICKERS
    except ImportError:
...
def parse_args():
    ...
    parser.add_argument('--compare', action='store_true',
                        help=f'與 {DEFAULT_BENCHMARK_LABEL} benchmark 比較')
...
def main():
    ...
    us_aligned = align_us_to_hk(us_signals, close_df.index)
    ...
    if args.compare:
        ...
        if benchmark_equity is not None and not benchmark_equity.empty:
            ...
            print(f"   {'':20s} | {'策略':>10s} | {DEFAULT_BENCHMARK_LABEL:>10s}")
            print(f"   {'─'*50}")

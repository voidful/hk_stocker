...
from strategy.market import from_yahoo_symbol, to_yahoo_symbol
...
def fetch_value_data(tickers, close_df):
    ...
    for i in range(0, len(hk_tickers), batch_size):
        batch = hk_tickers[i:i+batch_size]
        for hk_t in batch:
            try:
                info = yf.Ticker(hk_t).info
                t = from_yahoo_symbol(hk_t)

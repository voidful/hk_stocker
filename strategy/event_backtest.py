...
from strategy.market import DEFAULT_BUY_COST, DEFAULT_SELL_COST
...
class EventDrivenBacktester:
    ...
    buy_cost : float
        買進成本率（預設約 0.1127%，不含券商佣金）
    sell_cost : float
        賣出成本率（預設約 0.1127%，不含券商佣金）
    """
    ...
    def __init__(self, tp_pct=0.15, sl_pct=0.08, max_hold_days=20,
                 ...
                 tilt_strength=1.0,
                 tilt_windows=None,
                 buy_cost=DEFAULT_BUY_COST, sell_cost=DEFAULT_SELL_COST):
        self.tp_pct = tp_pct
        self.sl_pct = sl_pct
    ...
    def run(self, total_score, close_df, open_df, high_df, low_df, ma_60,
        ...
        market_close : pd.Series, optional
            大盤指數或 ETF 收盤價，用於 regime filter

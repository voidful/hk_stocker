...
計算趨勢方向、動量、恐慌水位等信號，作為港股板塊輪動策略的前提條件。

核心概念：
- 港股整體板塊跟隨美股大盤 (SPY) + 恐慌指數 (VIX)
- 港股互聯網科技前提參考費半 (SOX)
- 美股收盤 = 香港次日凌晨 → 港股 t 日用美股 t-1 日數據（天然領先）
...
def align_us_to_tw(us_signals, tw_dates):
    ...
    pd.DataFrame
        對齊後的信號，使用前一個可用的美股數據
    """
    # 將美股日期 shift +1 天（美股昨天 → 港股今天）
    # 然後用 forward fill 對齊到港股交易日
    aligned = us_signals.reindex(tw_dates, method='ffill')
    # Shift by 1 to use t-1 US data for t HK decision
    aligned = aligned.shift(1)
    return aligned


def align_us_to_hk(us_signals, hk_dates):
    """Alias with Hong Kong naming for new code."""
    return align_us_to_tw(us_signals, hk_dates)

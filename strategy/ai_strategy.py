...
from strategy.market import MARKET_NAME_ZH, from_yahoo_symbol, to_yahoo_symbol
...
def fetch_panel_data(tickers, days=800, start_date=None, end_date=None):
    """
    批次下載多檔港股的 OHLCV 日線資料。

    Parameters
    ----------
    tickers : list[str]
        港股代號列表，例如 ['0700', '0005', '1299']
    days : int
    ...
    close_df, open_df, high_df, low_df, vol_df : tuple[pd.DataFrame]
        各為 (日期 x 股票代號) 的 DataFrame。港股代號會正規化為四碼。
    """
    ...
    tickers = [from_yahoo_symbol(t) for t in tickers]

    print(f"📥 正在批次下載 {len(tickers)} 檔{MARKET_NAME_ZH}股票的歷史資料 "
    ...
    def _extract_field(raw_df, field):
        ...
        if isinstance(raw_df.columns, pd.MultiIndex):
            ...
            if len(tickers) == 1:
                extracted.columns = [tickers[0]]
        else:
            return pd.DataFrame(index=raw_df.index)
        extracted = extracted.copy()
        extracted.columns = [from_yahoo_symbol(c) for c in extracted.columns]
    ...
    hk_tickers = [to_yahoo_symbol(t) for t in tickers]
    all_dfs = _download_symbols(hk_tickers)
    ...
    if missing_tickers:
        ...
        if len(missing_tickers) > 8:
            preview += ", ..."
        print(f"   ⚠️ Yahoo 無資料或停牌過久: {len(missing_tickers)} 檔 ({preview})")

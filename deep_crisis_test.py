#!/usr/bin/env python3
"""
深度危機壓測 + 港股 benchmark 對標 + 弱勢分析

對 Sector Rotation v2 策略在所有已知歷史危機做完整壓測，
用 2800.HK 和 2828.HK 做 benchmark，並分析弱勢期間的指標特徵，
找出需要注意的信號和權重調整方向。
"""
...
def get_benchmark_return(ticker, start_date, end_date):
    ...
    try:
        data = yf.download(to_yahoo_symbol(ticker), start=start_date,
...
def main():
    print(f"{'='*80}")
    print(f"🔥 深度危機壓測 + 港股 benchmark 對標 — {datetime.now().strftime('%Y-%m-%d')}")
    print(f"{'='*80}")
    print(f"   策略: Sector Rotation v2 (美股前提 + 板塊資金流)")
    print(f"   對照: v8.5 Momentum + {DEFAULT_BENCHMARK_LABEL} + {SECONDARY_BENCHMARK_LABEL}")
    print()
    ...
    for name, info in CRISIS_PERIODS.items():
        ...
        # Benchmarks
        bm_0050 = get_benchmark_return(DEFAULT_BENCHMARK, eval_start, eval_end)
        bm_00981a = get_benchmark_return(SECONDARY_BENCHMARK, eval_start, eval_end)

        # US indicators
        us_ind = get_us_indicators(eval_start, eval_end)

        # 輸出
        print(f"\n   {'':16s} | {'SR v2':>10s} | {'v8.5':>10s} | {DEFAULT_BENCHMARK_LABEL:>10s} | {SECONDARY_BENCHMARK_LABEL:>10s}")
        print(f"   {'─'*70}")
    ...
    header = (f"  {'期間':<14s} | {'SR v2':>8s} | {'v8.5':>8s} | "
              f"{DEFAULT_BENCHMARK_LABEL:>8s} | {'SR MDD':>8s} | {'v85 MDD':>8s} | {'VIX':>5s}")
    print(header)
    print(f"  {'-'*76}")
    ...
    # ====== Secondary benchmark 共存期比較 ======
    coexist = [r for r in all_results if r['bm_00981a'] is not None]
    if coexist:
        print(f"\n\n{'='*80}")
        print(f"📊 {SECONDARY_BENCHMARK_LABEL} 共存期比較")
        print(f"{'='*80}")
        for r in coexist:
            sr_ann = r['sr']['ann']
            ...
            icon = '✅' if diff > 0 else '🔴'
            print(f"  {r['label']}: SR v2 {sr_ann:+.1f}% vs {SECONDARY_BENCHMARK_LABEL} {bm_ret:+.1f}% "
                  f"({icon} 差距 {diff:+.1f}%)")

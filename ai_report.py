#!/usr/bin/env python3
"""
AI 港股實戰區間交易系統 v2 (Event-Driven Quantitative Trading Pipeline)

完整管線：資料下載 → 動態 Universe → AI 特徵排名 → 事件驅動回測 → 風險分析 → HTML 報表

v2 改進：
- Entry 改為 t+1 open（對齊實盤）
- 動態 Liquid Universe（港股主要股票，按流動性篩選 Top-N）
- Top-K 選股取代固定 threshold
- Equity-based position sizing
- ATR 自適應 TP/SL
- 港股交易成本（印花稅 + 交易徵費 + 交易費 + 結算費）
- 完整風險指標（Sharpe/Sortino/MaxDD/Calmar）
- 2800.HK Benchmark 對比
- exchange_calendars 精確交易日

使用方式：
    python ai_report.py
    python ai_report.py --tickers 0700 9988 0005 --static-pool
...
def get_next_n_trading_days(from_date, n_days):
    ...
    if HAS_EXCHANGE_CAL and HK_CALENDAR is not None:
        try:
            ...
            sessions = HK_CALENDAR.sessions_in_range(from_ts, end_search)
...
def _build_inst_section():
    """
    建立港股籌碼動態 HTML section。
    港股版目前保留介面，待接入港股資金流資料源。
    """
    try:
        up_list = fetch_inst_rankings(20, 'up') or []
        down_list = fetch_inst_rankings(20, 'down') or []
    except Exception:
        return '<h2>🏛️ 港股籌碼動態</h2><p class="section-note">⚠️ 籌碼數據暫時無法取得</p>'

    if not up_list and not down_list:
        return '<h2>🏛️ 港股籌碼動態</h2><p class="section-note">⚠️ 港股資金流資料源尚未接入</p>'
...
def generate_report(trades_df, equity_df, total_score, close_df, config,
    ...
    if benchmark_equity is not None and len(benchmark_equity) > 0:
        ...
        if len(common_idx) > 0:
            ...
            ax1.plot(common_idx, bench_scaled, color='#ff9500', lw=1.5, alpha=0.85,
                     label=f'{DEFAULT_BENCHMARK_LABEL} Buy & Hold', linestyle='--')
    ...
    if benchmark2_equity is not None and len(benchmark2_equity) > 0:
        ...
        if len(common_idx) > 0:
            ...
            ax1.plot(common_idx, bench2_scaled, color='#34c759', lw=1.5, alpha=0.85,
                     label=f'{SECONDARY_BENCHMARK_LABEL} Buy & Hold', linestyle='-.')
    ...
    <div class="stats">
        <div class="stat-card benchmark">
            <div class="label">{DEFAULT_BENCHMARK_LABEL} 年化報酬</div>
            <div class="value">{bm_ann:+.1f}%</div>
        </div>
        <div class="stat-card benchmark">
            <div class="label">{DEFAULT_BENCHMARK_LABEL} 最大回撤</div>
            <div class="value">{bm_mdd:.1f}%</div>
        </div>
        <div class="stat-card benchmark">
            <div class="label">{DEFAULT_BENCHMARK_LABEL} Sharpe</div>
    ...
    <div class="stats">
        <div class="stat-card benchmark">
            <div class="label">{SECONDARY_BENCHMARK_LABEL} 年化報酬</div>
            <div class="value">{bm2_ann:+.1f}%</div>
        </div>
        <div class="stat-card benchmark">
            <div class="label">{SECONDARY_BENCHMARK_LABEL} 最大回撤</div>
            <div class="value">{bm2_mdd:.1f}%</div>
        </div>
        <div class="stat-card benchmark">
            <div class="label">{SECONDARY_BENCHMARK_LABEL} Sharpe</div>
            <div class="value">{bm2_sharpe:.2f}</div>
        </div>
        <div class="stat-card" style="border-left-color:{e2_color}">
            <div class="label">超額 vs {SECONDARY_BENCHMARK_LABEL} (共存期)</div>
...
{rows_html}
    ...
    <div class="stats">
        ...
        <div class="stat-card benchmark">
            <div class="label">{DEFAULT_BENCHMARK_LABEL} 近 5 日</div>
            <div class="value" style="color:{'#00ff00' if ret_5d > 0 else '#ff4444'}">{ret_5d:+.1f}%</div>
        </div>
        <div class="stat-card benchmark">
            <div class="label">{DEFAULT_BENCHMARK_LABEL} 近 20 日</div>
            <div class="value" style="color:{'#00ff00' if ret_20d > 0 else '#ff4444'}">{ret_20d:+.1f}%</div>
        </div>
        <div class="stat-card benchmark">
            <div class="label">{DEFAULT_BENCHMARK_LABEL} 近 60 日</div>
...
{top_dd_rows}
    ...
    cost_desc = f"買 {config.get('buy_cost', DEFAULT_BUY_COST)*100:.3f}% + 賣 {config.get('sell_cost', DEFAULT_SELL_COST)*100:.3f}%"
    ...
    risk_notes = [
        "Regime ON" if config.get('regime_filter', False) else "Regime OFF",
        "Corr OFF" if config.get('corr_filter', 0.0) <= 0 else f"Corr<{config.get('corr_filter', 0.0):.2f}",
    ]
    if config.get('dd_pause_pct', 1.0) < 1.0:
        risk_notes.append(f"DD pause {config.get('dd_pause_pct'):.0%}")
    if config.get('consec_loss_limit', 99) < 99:
        risk_notes.append(f"Loss pause {config.get('consec_loss_limit')}x")
    risk_desc = " + ".join(risk_notes)
...
<head>
    ...
    <title>AI 港股量化交易 v8.5 — {report_date}</title>
    <meta name="description" content="AI 驅動的港股量化交易系統 v8.5，完整風險報告、Benchmark 對比、OCO 智慧掛單建議">
    <script>
        (function() {{
            let savedTheme = null;
            try {{ savedTheme = localStorage.getItem('hk-stocker-theme'); }} catch (error) {{}}
...
<div class="container">
    ...
    <h1>🎯 AI 港股量化交易 v8.5</h1>
    <p class="subtitle">
        Event-Driven System &nbsp;|&nbsp; 報表日期: {report_date} &nbsp;|&nbsp;
        <span class="config-badge">🛡️ {mode_html}</span>
        <span class="config-badge">🌐 Universe-{config.get('universe_size', '-')}</span>
        <span class="config-badge">🎯 Top-{top_k}</span>
        <span class="config-badge">🧠 RSI×{config.get('rsi_weight', 0):.1f}</span>
    ...
    <p class="section-note">
        ...
        {f'最晚出場日使用 {EXCHANGE_CALENDAR} 交易日曆計算' if HAS_EXCHANGE_CAL else '最晚出場日為近似值（未安裝 exchange_calendars）'}
    </p>
    <table>
        <thead>
            <tr>
...
{monthly_heatmap_html}
    ...
    <div class="disclaimer">
        ...
        成本 = {cost_desc} | 回測期 = {m['years']:.1f} 年 | 因子 = Mom(20d)×3 + Trend(60MA)×1 + RSI×{config.get('rsi_weight', 0):.1f} |
        風控 = {risk_desc}
    </div>
...
<script>
    (function() {{
        ...
        function setTheme(theme) {{
            ...
            try {{ localStorage.setItem('hk-stocker-theme', nextTheme); }} catch (error) {{}}
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='AI 港股量化交易系統 v8.5 — 事件驅動回測與交易計畫產生器'
    )
    # 股池
    parser.add_argument(
    ...
    # 選股
    parser.add_argument(
        '--top-k', type=int, default=12,
        help='每日最多進場股票數 (預設: 12)'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=f'大盤過濾 ({DEFAULT_BENCHMARK_LABEL} > 60MA 才允許進場, 預設: OFF)'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--gap-filter', type=float, default=0.0,
        help='跳空過濾 ATR 倍數 (預設: 0=停用)'
    )
    parser.add_argument(
    ...
    # 成本
    parser.add_argument(
        '--buy-cost', type=float, default=DEFAULT_BUY_COST,
        help=f'買入成本率 (預設: {DEFAULT_BUY_COST:.6f}，約 {DEFAULT_BUY_COST*100:.3f}%%，不含券商佣金)'
    )
    parser.add_argument(
        '--sell-cost', type=float, default=DEFAULT_SELL_COST,
        help=f'賣出成本率 (預設: {DEFAULT_SELL_COST:.6f}，約 {DEFAULT_SELL_COST*100:.3f}%%，不含券商佣金)'
    )
    ...
    parser.add_argument(
        ...
        help='啟用恒指期空單對沖（熊市時模擬做空大盤）'
    )

    # 風控節流：港股版預設啟用，避免連續失利時過度交易。
    parser.add_argument(
        '--dd-pause-pct', type=float, default=0.10,
        help='權益回撤暫停門檻 (預設 0.10; 1.0 = 停用)'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--consec-loss-limit', type=int, default=3,
        help='連續停損次數上限 (預設 3; 99 = 停用)'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help='相關性過濾門檻 (預設 0 = 停用; >0 會去除近 20 日高相關重複持倉)'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help='在報表信號中顯示籌碼與新聞情緒標注 (港股資料源尚未接入，預設開啟)'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--rsi-weight', type=float, default=1.5,
        help='RSI-20 動量因子權重 (預設 1.5)'
    )
    parser.add_argument(
        '--breakout-weight', type=float, default=0.0,
        help='300日創新高突破因子權重 (預設 0 = 停用)'
    )
    parser.add_argument(
...
def main():
    ...
    # Phase 3.5: 提前下載 benchmark 用於 regime filter + 殘差動量
    market_close = None
    if args.regime_filter or args.residual_momentum:
        print(f"\n📊 下載大盤指數 ({DEFAULT_BENCHMARK_LABEL}) 用於 regime filter...")
        bench_raw = fetch_benchmark(
            DEFAULT_BENCHMARK, days=args.days,
            start_date=args.start_date, end_date=args.end_date,
        )
        if len(bench_raw) > 0:
    ...
    benchmark2_equity = fetch_benchmark(
        SECONDARY_BENCHMARK, days=args.days,
        start_date=benchmark_start, end_date=args.end_date,
    )
